...
app.get('/api/centres', async (req, res) => {
  ...
  let criteria = `(Ville:equals:${query}) or (D_partement:equals:${query}) or (Code_postal:equals:${query}) and (Layout.id:equals:6266060000000091029)`;

  try {
    const accessToken = await getAccessToken();
    const url = 'https://www.zohoapis.com/crm/v2/Accounts/search';
    const response = await axios.get(url, {
      headers: { 'Authorization': `Zoho-oauthtoken ${accessToken}` },
      params: { criteria }
    });
    ...
    if (response.data.data) {
      ...
      console.error("No data found for the criteria:", criteria);
      res.status(404).send('Aucun centre trouvé.');
    }
  } catch (error) {
    console.error('Erreur lors de la récupération des données', error);
